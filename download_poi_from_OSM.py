...
for filter_tag in filter_tags:
    ...
    if not os.path.exists(target_path):
        print("no file with this tag, let's download")
    ...
    if should_it_be_downloaded:
        ...
        if cur_tags_poi is not None:
            ...
            lib_over.save_data_as(cur_tags_poi, target_path, format='csv')
...
print(f"Finished downloading {total_poi} POIs\nDownloaded {len(downloaded_tags)}/{len(filter_tags)} tags")
